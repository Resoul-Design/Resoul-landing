...
--       匿名提交、預先審核（held→visible）、公開或只限連結、留下心意（heart）。
-- 本檔可重複執行（idempotent）。
-- ============================================================
...
alter table public.posts add column if not exists visibility text not null default 'public'; -- public/link
...
do $$ begin
  alter table public.posts drop constraint if exists visibility_val;
  update public.posts set visibility = 'link' where visibility = 'private';
  alter table public.posts add constraint visibility_val check (visibility in ('public','link'));
...
begin
  ...
  if new.visibility is null or new.visibility not in ('public','link') then
...
begin
  update public.posts
     set hearts = hearts + 1
   where id = p_id and status = 'visible' and visibility in ('public','link')
